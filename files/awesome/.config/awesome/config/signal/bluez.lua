...
function bluetooth:toggle()
  ...
  self._private.adapter_proxy:Set(
    'org.bluez.Adapter1',
    'Powered',
    ...
    if device_proxy.Name ~= '' and device_proxy.Name ~= nil then
      device_properties_proxy:connect_signal('PropertiesChanged', function(_, _, changed_properties)
        for key, _ in pairs(changed_properties) do
          -- naughty.notify {
          --   title = 'bluez signal',
          --   message = gdebug.dump_return(changed_properties),
          -- }
...
local function new()
  ...
  ret._private.object_manager_proxy = dbp.Proxy:new {
    bus = dbp.Bus.SYSTEM,
    name = 'org.bluez',
  ...
  ret._private.adapter_proxy = dbp.Proxy:new {
    bus = dbp.Bus.SYSTEM,
    name = 'org.bluez',
  ...
  ret._private.adapter_proxy_properties = dbp.Proxy:new {
    bus = dbp.Bus.SYSTEM,
    name = 'org.bluez',
    interface = 'org.freedesktop.DBus.Properties',
    path = '/org/bluez/hci0',
  ...
  ret._private.adapter_proxy_properties:connect_signal(
    'PropertiesChanged',
    function(self, interface, data)
