...
local debounce_trailing = function(fn, ms)
...
local function create_highlight(rgb_hex, options)
  ...
  if mode == 'foreground' then
    ...
    vim.api.nvim_command(
      string.format('highlight %s guifg=%s guibg=#%s', highlight_name, fg_color, rgb_hex)
    )
  end
...
function M.update_highlight(client, bufnr, options)
...
function M.buf_attach(client, bufnr, options)
  ...
  local trigger_update_highlight, timer = debounce_trailing(M.update_highlight, options.debounce)
  ...
  trigger_update_highlight(client, bufnr, options)
