...
local function on_wifi_device_state_changed(self, proxy, new_state, old_state, reason)
  local active_access_point_proxy = dbp.Proxy:new {
    bus = dbp.Bus.SYSTEM,
...
local function get_access_point_connections(self, ssid)
  ...
  for _, connection_path in ipairs(connections) do
    local connection_proxy = dbp.Proxy:new {
      bus = dbp.Bus.SYSTEM,
      name = 'org.freedesktop.NetworkManager',
...
local function get_wifi_proxy(self)
  ...
  for _, device_path in ipairs(devices) do
    local device_proxy = dbp.Proxy:new {
      bus = dbp.Bus.SYSTEM,
      name = 'org.freedesktop.NetworkManager',
    ...
    if device_proxy.DeviceType == network.DeviceType.WIFI then
      ...
      self._private.wifi_proxy = dbp.Proxy:new {
        bus = dbp.Bus.SYSTEM,
        name = 'org.freedesktop.NetworkManager',
...
function network:scan_access_points()
  ...
  self._private.wifi_proxy:RequestScanAsync(function(proxy, context, success, failure)
    ...
    for _, access_point_path in ipairs(access_points) do
      local access_point_proxy = dbp.Proxy:new {
        bus = dbp.Bus.SYSTEM,
        name = 'org.freedesktop.NetworkManager',
        interface = 'org.freedesktop.NetworkManager.AccessPoint',
...
function network:connect_to_access_point(access_point, password, auto_connect)
  ...
  if #connections == 0 then
    ...
    self._private.client_proxy:AddAndActivateConnectionAsync(
      ...
      { call_id = 'my-id' },
      profile,
      access_point.device_proxy_path,
      access_point.path
    )
  else
...
function network:toggle_wireless_state()
  ...
  self._private.client_proxy:Set(
    'org.freedesktop.NetworkManager',
    'WirelessEnabled',
    lgi.GLib.Variant('b', enable)
  )
...
local function new()
  ...
  ret._private.client_proxy = dbp.Proxy:new {
    bus = dbp.Bus.SYSTEM,
    name = 'org.freedesktop.NetworkManager',
  ...
  ret._private.settings_proxy = dbp.Proxy:new {
    bus = dbp.Bus.SYSTEM,
    name = 'org.freedesktop.NetworkManager',
  ...
  local client_properties_proxy = dbp.Proxy:new {
    bus = dbp.Bus.SYSTEM,
    name = 'org.freedesktop.NetworkManager',
  ...
  gtimer.delayed_call(function()
    ...
    if
      ret._private.device_proxy.State == network.DeviceState.ACTIVATED
      and active_access_point ~= '/'
    then
      local active_access_point_proxy = dbp.Proxy:new {
        bus = dbp.Bus.SYSTEM,
        name = 'org.freedesktop.NetworkManager',
