local color_lib = require('module.color')
...
function M.is_dark(hex)
  local color = color_lib.color { hex = hex }

  return color.l <= 0.4
end

--- Check if a color is opaque.
function M.is_opaque(hex)
  local color = color_lib.color { hex = hex }
...
function M.rotate_hue(color, angle)
  color = color_lib.color { hex = color }
...
function M.button_color(hex, amount)
  local color = color_lib.color { hex = hex }
...
function M.pywal_blend(color1, color2)
  color1 = color_lib.color { hex = color1 }
  color2 = color_lib.color { hex = color2 }
...
function M.pywal_saturate_color(hex, amount)
  local color = color_lib.color { hex = hex }
...
function M.pywal_alter_brightness(hex, amount, sat)
  sat = sat or 0
...
function M.pywal_lighten(hex, amount)
  local color = color_lib.color { hex = hex }
...
function M.pywal_darken(hex, amount)
  local color = color_lib.color { hex = hex }
